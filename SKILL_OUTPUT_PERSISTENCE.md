...
Neither mode permits replacement.
...
replacement, and syncs its parent directory.
...
run rejects every later write or state transition. Retry uses a new run ID and
new artifact paths.
...
are rejected when internet is disabled. Receipt `internet.used` is derived from
