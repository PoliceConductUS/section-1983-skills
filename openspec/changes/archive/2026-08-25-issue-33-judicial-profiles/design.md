...
contract maps each operation name to its exact internet policy: `authorized` for
acquisition and `disabled` for compilation. Each invocation declares one
operation and one policy, and trusted-host validation rejects a missing or
unknown operation and every wrong operation-policy pair before resolving inputs.
Existing single-operation contracts keep their exact scalar policy and do not
add an operation field.
