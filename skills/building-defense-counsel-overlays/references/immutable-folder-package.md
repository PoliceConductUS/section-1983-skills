...
- `package-manifest.json` records package identity, kind, UTC creation time,
  ...
  receipt identity.
- Every non-manifest regular file outside trusted-host `.skill-runs/` and
  `temp/` control namespaces must appear exactly once as a hashed member.
...
- The caller supplies the complete absolute output-folder path. Only the trusted
  host publishes every member and `package-manifest.json` directly in that
  folder. It never adds a registry namespace or mutates an input package.
- The existing counsel snapshot and overlay schemas continue to decide domain
  validity; the folder envelope decides package membership and integrity.
